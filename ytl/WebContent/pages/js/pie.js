...
window.onload = function() {
	...
	tr.each(function(){
		...
		var td=$(this).find("td");
		var data={name:td.eq(0).text(),value:parseInt(td.eq(2).text()),color:"RGBA("+rgba()+","+rgba()+","+rgba()+",1)"};
...
/*window.onload = function() {
				...
				tr.each(function(){
					...
					let td=$(this).find("td");
					data={name:td.eq(0).text(),parseInt(value:td.eq(2).text()),color:"RGBA("+rgba()+","+rgba()+","+rgba()+",1)"};
					seriesData.push(data);
				});
				console.log(seriesData);
				var seriesData1 = [{
